...
import SongFullScreen from "./components/SongFullScreen.vue"
...
const routes = [
    {
        path: "/fullscreen/:song_title_lookup",
    ...
    getters: {
        playlists(state) {
            return state.all_playlists
                .filter(
                    (t) => t.playlist_date.getDay() == state.weekday
                )
                .sort((b, a) => a.playlist_date - b.playlist_date)
        },
        is_sunday(state) {
